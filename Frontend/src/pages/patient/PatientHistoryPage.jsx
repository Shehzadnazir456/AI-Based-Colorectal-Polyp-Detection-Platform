  ...
  return (
    <div>
      ...
      {!loading && entries.length === 0 ? (
        <EmptyState title="No history entries" description="Your provider will add visit notes here." />
      ) : (
        <div className="relative space-y-4 before:absolute before:left-4 before:top-2 before:h-[calc(100%-16px)] before:w-px before:bg-violet-200">
          {entries.map((h, i) => (
            <GlassCard key={h.id ?? i} className="relative !p-4 pl-12">
              ...
              <p className="mt-2 text-sm text-slate-800">{h.notes || h.note || h.description || JSON.stringify(h)}</p>
            </GlassCard>
          ))}
        </div>
      )}
